...
import { Download, CheckCircle, Clock, XCircle, RefreshCw, Globe } from 'lucide-react'
...
export default function JobsList() {
  ...
  useEffect(() => {
    fetchJobs()
    const interval = setInterval(fetchJobs, 2000) // Atualizar a cada 2 segundos
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-400 mt-2">
          Faça upload de um arquivo na aba "Novo Upload"
        </p>
      </div>
    )
  }
                ...
                {job.status === 'completed' && job.download_urls && (
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleDownload(job.download_urls!.json, `${job.filename}.json`)}
                        className="inline-flex items-center px-3 py-1 border border-gray-300 rounded-md text-xs font-medium text-gray-700 bg-white hover:bg-gray-50"
                      >
                        <Download className="w-3 h-3 mr-1" />
                        JSON
